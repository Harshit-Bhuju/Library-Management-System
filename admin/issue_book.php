                    ...
                    <form method="POST" class="space-y-6" id="issueForm">
                        ...
                        <div>
                            ...
                            <input type="date" name="due_date" class="form-control"
                                value="<?php echo date('Y-m-d', strtotime("+16 days")); ?>"
                                min="<?php echo date('Y-m-d'); ?>"
                                required>
                            <p class="text-xs text-gray-500 mt-1">Default: 15 days from tomorrow</p>
                        </div>
                ...
                <div class="stat-card" data-aos="fade-up" data-aos-delay="100">
                    ...
                    <ul class="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                        ...
                        <li class="flex items-start gap-2">
                            ...
                            <span>Fine: <?php echo CURRENCY_SYMBOL . ' ' . getSetting('fine_per_day', DEFAULT_FINE_PER_DAY); ?> per day after due date</span>
                        </li>
...
<script>
    $(document).ready(function() {
        ...
        // User selection info (using Select2 change event)
        $('#userSelect').on('change', function() {
            ...
            if (selected.value) {
                ...
                const limit = <?php echo getSetting('max_books_student', defined('MAX_BOOKS_STUDENT') ? MAX_BOOKS_STUDENT : 3); ?>;
        ...
        // Book selection info (using Select2 change event)
        ...
        $('#issueForm').on('submit', function(e) {
            const user = $('#userSelect option:selected').text().split('(')[0].trim();
            const book = $('#bookSelect option:selected').text().split('by')[0].trim();
            if (user && book) {
                confirmAction(e, `Are you sure you want to issue "${book}" to ${user}?`, 'Confirm Issuance', 'primary');
            }
        });
    });
</script>
